...
def epf_calculation(name,carry_forward_amount):
    ...
    date_sequence = pd.date_range(start='2026-03-01', end='2048-03-01', freq='MS')
    ...
    """
    df['Basic'] = np.nan
    df.loc[df['Month'] == '2026-03-01', 'Basic'] = 72240

    df['VPFPercent'] = np.nan
    df.loc[df['Month'] == '2026-03-01', 'VPFPercent'] = 0.23

    df['EESPFPercent'] = np.nan
    df.loc[df['Month'] == '2026-03-01', 'EESPFPercent'] = 0.12

    df['ERSPFPercent'] = np.nan
    df.loc[df['Month'] == '2026-03-01', 'ERSPFPercent'] = 0.08

    df['RatePerAnnum'] = np.nan
    df.loc[df['Month'] == '2026-03-01', 'RatePerAnnum'] = 0.0825
